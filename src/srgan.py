...
class SRGAN:
    ...
    def generator(self, x, is_training, reuse):
        with tf.compat.v1.variable_scope('generator', reuse=reuse):
            ...
            for i in range(5):
                mid = x
                with tf.compat.v1.variable_scope('block{}a'.format(i+1)):
                    x = deconv_layer(
                        x, [3, 3, 64, 64], [self.batch_size, 24, 24, 64], 1)
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.compat.v1.variable_scope('block{}b'.format(i+1)):
                    x = deconv_layer(
                        x, [3, 3, 64, 64], [self.batch_size, 24, 24, 64], 1)
                    x = batch_normalize(x, is_training)
                x = tf.add(x, mid)
            ...
            with tf.compat.v1.variable_scope('deconv3'):
                ...
                x = tf.nn.relu(x)
            with tf.compat.v1.variable_scope('deconv4'):
                x = deconv_layer(
                    x, [3, 3, 64, 64], [self.batch_size, 48, 48, 64], 1)
                x = pixel_shuffle_layer(x, 2, 16)
                x = tf.nn.relu(x)
            with tf.compat.v1.variable_scope('deconv5'):
                x = deconv_layer(
    ...
    def discriminator(self, x, is_training, reuse):
        ...
        self.d_variables = tf.compat.v1.get_collection(
